...
public partial class InvoicePreviewWindow : Window
{
    ...
    private void Star_Click(object sender, RoutedEventArgs e)
    {
        ...
        MessageBox.Show(_isStarred ? "فاکتور ستاره‌دار شد" : "ستاره برداشته شد", "ستاره",
    ...
    private void Print_Click(object sender, RoutedEventArgs e)
    {
        try
        {
            ...
            doc.PageWidth = 842; // A4 landscape width
            doc.PageHeight = 595; // A4 landscape height
            doc.PagePadding = new Thickness(40);

            // Shop name
            var titlePara = new Paragraph(new Run(s.ShopName ?? "فروشگاه"))
            {
                FontSize = 20, FontWeight = FontWeights.Bold,
                Foreground = new SolidColorBrush(Color.FromRgb(0, 16, 120)),
            ...
            // Invoice info
            doc.Blocks.Add(new Paragraph(new Run($"شماره فاکتور: {_sale.InvoiceNumber}"))
            { FontSize = 10, Foreground = Brushes.Gray, TextAlignment = TextAlignment.Center });
            doc.Blocks.Add(new Paragraph(new Run($"تاریخ: {_sale.CreatedAt:yyyy/MM/dd HH:mm}"))
            { FontSize = 10, Foreground = Brushes.Gray, TextAlignment = TextAlignment.Center, Margin = new Thickness(0, 0, 0, 10) });

            doc.Blocks.Add(new Paragraph(new Run("────────────────────────────────") { Foreground = Brushes.LightGray, FontSize = 8 }));
            ...
            foreach (var header in new[] { "کالا", "تعداد", "قیمت", "جمع" })
            ...
            foreach (var item in items)
            {
                ...
                foreach (var val in new[] { item.ProductTitle, item.Quantity.ToString(), $"{item.UnitPrice:N0}", $"{item.Subtotal:N0}" })
            ...
            // Totals
            doc.Blocks.Add(new Paragraph(new Run($"جمع کل: {_sale.Subtotal:N0} ریال") { FontSize = 11 })
            { TextAlignment = TextAlignment.Left, Margin = new Thickness(0, 0, 0, 3) });
            doc.Blocks.Add(new Paragraph(new Run($"مالیات: {_sale.TaxAmount:N0} ریال") { FontSize = 10, Foreground = Brushes.Gray })
            { TextAlignment = TextAlignment.Left, Margin = new Thickness(0, 0, 0, 5) });
            doc.Blocks.Add(new Paragraph(new Run($"مبلغ قابل پرداخت: {_sale.TotalAmount:N0} ریال") { FontSize = 14, FontWeight = FontWeights.Bold })
            { TextAlignment = TextAlignment.Left, Margin = new Thickness(0, 5, 0, 10) });

            doc.Blocks.Add(new Paragraph(new Run("────────────────────────────────") { Foreground = Brushes.LightGray, FontSize = 8 }));
            doc.Blocks.Add(new Paragraph(new Run(s.ReceiptFooter ?? "با تشکر از خرید شما"))
            { FontSize = 10, Foreground = Brushes.Gray, TextAlignment = TextAlignment.Center });
